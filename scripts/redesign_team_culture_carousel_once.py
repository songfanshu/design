...
for rel in ['team-culture/academic.html','team-culture/collaboration.html','team-culture/community.html']:
    p = Path(rel)
    s = p.read_text(encoding='utf-8')
    s = s.replace('<a href="growth.html">共同成长 →</a>', '')
    p.write_text(s, encoding='utf-8')

growth = Path('team-culture/growth.html')
if growth.exists():
    growth.unlink()
